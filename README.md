...
### 5. GitHub 本月热门
- 拉取近一个月内新建、按星标排序的 **Top 10** 仓库，点击直达。
- 与上次记录对比，**新进榜的仓库标记「新上榜」**。
- 6 小时本地缓存，避免频繁请求；失败时保留旧数据并提供重试。

### 6. 主题与本地化
- **四套配色方案**（多巴胺 / 糖果 / 日出 / 薄荷）：切换会同步改变品牌撞色、强调色与背景，而非仅换浅色底。
- 浅色 / 深色 / 跟随系统主题。
- **中英文双语一等公民**：所有按钮、提示、文案、图表标签统一走 i18n 字典，不留死角。
